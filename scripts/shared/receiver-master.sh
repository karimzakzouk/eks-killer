#!/usr/bin/env bash
# Runs as a long-lived service on any node that is eligible to become the
# new control plane: replacement masters and (via receiver-worker.sh) workers.
# pyreceiver.py is already started by boot userdata on port HANDOFF_PORT and
# atomically writes the bundle to INCOMING once it is fully received AND
# (for framed transfers) checksum-verified. This script polls for that
# file and, once present, drives the etcd-restore + manifest-rewrite +
# EIP-reassociation promotion sequence. The framed-protocol validation
# already ran inside pyreceiver before this script ever sees the file.
